...
class PieceClass:
    ...
    def __init__(self, filePath):
        ...
        self.m21piece = m21.converter.parse(filePath)
        self.m21piece.quantize([4], processOffsets=True, processDurations=True, inPlace=True)
        self.notes = self.m21piece.flat.notes
        # get important info
        self.pitches = np.array( [m.pitch.midi for m in self.notes] )
        self.durations = np.array( [d.duration.quarterLength for d in self.notes] )
        self.offsets = np.array( [f.offset for f in self.notes] )
        # transpose to normalised key
        self.key = self.m21piece.analyze('key')
        # self.key.show('text')
